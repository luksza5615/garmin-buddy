...
class AnalysisService:
    ...
    def calculate_kpis(self, activities: pd.DataFrame) -> dict[str, float]:
        ...
        if "aerobic_training_effect_0_to_5" in activities.columns:
            kpis["aerobic_training_effect_0_to_5"] = float(
                activities["aerobic_training_effect_0_to_5"].mean()
            )
        if "anerobic_training_effect_0_to_5" in activities.columns:
            kpis["anaerobic_training_effect_0_to_5"] = float(
                activities["anaerobic_training_effect_0_to_5"].mean()
            )
    ...
    def calculate_basic_metrics(
        self, start_date: date = _DEFAULT_START_DATE, end_date: date = _DEFAULT_END_DATE
    ) -> dict[str, Any]:
        ...
        metrics = {
            ...
            "avg_running_efficiency_index": running_df[
                "running_efficiency_index"
            ].mean(),
            "aerobic_training_effect_0_to_5": running_df[
                "aerobic_training_effect_0_to_5"
            ].mean(),
            "anaerobic_training_effect_0_to_5": running_df[
