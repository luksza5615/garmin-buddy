...
@app.get("/")
def home():
    print("API works on Azure correctly!!!")
...
def fetch_latest_activity_date(connection):
    return connection.cursor().execute(
        "SELECT TOP 1 CONVERT(DATE, timestamp) AS LAST_DATE FROM dbo.activity_data ORDER BY timestamp DESC"
    ).fetchone()[0]
...
def refresh_db(connection):
    files = download_activities(connection, True)
    print(files)

    for path in files:
        parse_and_save_single_file_to_db(path, connection)
...
def get_activities():
    ...
    print(query)
    activities = pd.read_sql_query(query, engine)
    print('SQL done')
    return activities
...
# print(fetch_latest_activity_data(azure_connection))

# parse_fit_file(fit_file_path_test)

# refresh_activities()

# execute_azure(True, False)
...
# azure_connection = create_engine(db_connection_string_alchemy)
